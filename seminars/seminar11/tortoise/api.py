...
class TextToSpeech:
    ...
    def get_conditioning_latents(self, voice_samples, return_mels=False):
        ...
        with torch.no_grad():
            ...
            for sample in voice_samples:
                ...
                resample = torchaudio.transforms.Resample(orig_freq=22050, new_freq=24000)
    ...
    @staticmethod
    def _require_conditioning_latents(conditioning_latents):
        if conditioning_latents is None:
            raise ValueError("conditioning_latents must be provided for latent-based synthesis.")
        if not isinstance(conditioning_latents, (tuple, list)) or len(conditioning_latents) != 2:
            raise ValueError(
                "conditioning_latents must be a 2-item tuple/list of (autoregressive_latent, diffusion_latent)."
            )
        auto_conditioning, diffusion_conditioning = conditioning_latents
        if auto_conditioning is None:
            raise ValueError(
    ...
    def tts(self, text, voice_samples=None, conditioning_latents=None, k=1, verbose=True, use_deterministic_seed=None,
        ...
        if voice_samples is not None:
            ...
            auto_conditioning, diffusion_conditioning = self._require_conditioning_latents(conditioning_latents)
        else:
